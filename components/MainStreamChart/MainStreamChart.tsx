...
interface MainStreamChartProps {
  width: number
  height: number
  animate?: boolean
}

export const MainStreamChart = ({ width, height, animate = true }: MainStreamChartProps) => {
  ...
  const layers = transpose<number>(keys.map(() => generateData(SAMPLES_PER_LAYER, BUMPS_PER_LAYER)))
            ...
            stacks.map(stack => {
              ...
              return (
                <g key={`series-${stack.key}`}>
                  <path d={tweened.pathString} fill={color} />
                  <path d={tweened.pathString} fill={`url(#${pattern})`} />
                </g>
              )
            })
          }
