...
export default async function getAllTimeAggregations(
  ...
  if (req.method === 'GET') {
    try {
      ...
      const preBalance =
        (firstTransaction.balance || 0) - (firstTransaction.amount || 0);
      ...
      const totalPlusPercentage =
        (Math.abs(totalPlus) / (Math.abs(totalPlus) + Math.abs(totalMinus))) *
          100 || 0;
      const totalMinusPercentage =
        (Math.abs(totalMinus) / (Math.abs(totalPlus) + Math.abs(totalMinus))) *
          100 || 0;
      ...
      const categories = categoriesRaw.map(c => ({
        ...c,
        transactionBalance: Math.abs(
          c.transactions.reduce((sum, t) => (sum += t.amount || 0), 0)
        )
      }));
